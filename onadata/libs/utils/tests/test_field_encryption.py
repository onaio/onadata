...
class FieldEncryptionTestCase(SimpleTestCase):
    ...
    def test_a_malformed_key_raises_field_encryption_error(self):
        """Fernet() raises ValueError on a bad key, not InvalidToken; it must
        surface as FieldEncryptionError so callers that guard on it fail
        closed rather than 500."""
        with self.assertRaises(FieldEncryptionError):
            encrypt("v", keys=["not-a-valid-fernet-key"])
        with self.assertRaises(FieldEncryptionError):
            decrypt("anything", keys=["not-a-valid-fernet-key"])
