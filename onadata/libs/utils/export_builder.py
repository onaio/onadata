...
def dict_to_joined_export(data, index, indices, name, survey, row, media_xpaths=[]):
    """
        ...
        for (key, val) in iteritems(data):
            if isinstance(val, list) and key not in [NOTES, ATTACHMENTS, TAGS]:
                ...
                if key in [TAGS]:
                    ...
                    note_list = [v if isinstance(v, text) else v["note"] for v in val]
                    output[name][key] = "\r\n".join(note_list)
                else:
...
def is_all_numeric(items):
    ...
    try:
        for i in items:
            ...
            if isinstance(i, text) and len(i) > 1 and i[0] == "0" and i[1] != ".":
                return False
        return True
    except ValueError:
        return False
...
def track_task_progress(additions, total=None):
    ...
    try:
        ...
        logging.exception(_("Track task progress threw exception: %s" % text(e)))
...
class ExportBuilder(object):
    ...
    def _get_select_mulitples_choices(
        self, child, dd, field_delimiter, remove_group_name
    ):
            ...
            for child in survey_element.children:
                ...
                if isinstance(child, Section):
                    ...
                    if isinstance(child, Question):
                        ...
                        _label = (
                            dd.get_label(child_xpath, elem=child, language=language)
                            or _title
                        )
    ...
    def split_select_multiples(
        ...
        for (xpath, choices) in iteritems(select_multiples):
            ...
            if data:
                selections = [
                    "{0}/{1}".format(xpath, selection) for selection in data.split()
                ]
            ...
            for key in self.select_ones[section_name]:
                if key in row:
                    row[key] = get_choice_label_value(
                        key, row[key], self.dd, self.language
                    )
    ...
    def to_zipped_csv(self, path, data, *args, **kwargs):
        ...
        if not self.INCLUDE_LABELS_ONLY:
            for section in self.sections:
                ...
                csv_defs[section["name"]]["csv_writer"].writerow([f for f in fields])
        ...
        for i, d in enumerate(data, start=1):
            ...
            joined_export = dict_to_joined_export(
                d, index, indices, survey_name, self.survey, d, media_xpaths
            )
    ...
    def _get_sav_value_labels(self, xpath_var_names=None):
        ...
        for q in choice_questions:
            if xpath_var_names and q.get_abbreviated_xpath() not in xpath_var_names:
                continue
            var_name = (
                xpath_var_names.get(q.get_abbreviated_xpath())
                if xpath_var_names
                else q["name"]
            )
        ...
        var_types = dict(
            [
                (
                    ...
                    SAV_NUMERIC_TYPE
                    if _is_numeric(element["xpath"], element["type"], self.dd)
            ...
            + [
                (
                    x[1],
                    SAV_NUMERIC_TYPE
                    if _is_numeric(x[0], _get_element_type(x[0]), self.dd)
                    else SAV_255_BYTES_TYPE,
                )
