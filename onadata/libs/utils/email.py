...
def get_two_factor_email_data(
    username: str, event: str, **context: object
) -> dict[str, str]:
    ...
    if not support_email:
        raise ImproperlyConfigured(
            "SUPPORT_EMAIL must be set to send two-factor notification emails."
        )
    ...
    return {
        "subject": render_to_string(
            f"two_factor/{event}_email_subject.txt", ctx_dict
        ),
...
def send_generic_email(email, message_txt, subject, message_html=None):
    ...
    if not (email and message_txt and subject):
        raise ValueError("email, message_txt and subject arguments are ALL required.")
