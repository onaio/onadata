...
def _cipher(keys=None):
    ...
    try:
        ...
        # A malformed key raises ValueError from Fernet(), not InvalidToken;
        # funnel it through the fail-closed type so callers that guard on
        # FieldEncryptionError (verify_token) reject rather than 500.
        raise FieldEncryptionError(
            "A configured encryption key is not a valid Fernet key."
        ) from exc


@sensitive_variables()
def encrypt(value: str | None, keys: list[str] | None = None) -> str | None:
    if value is None:
        return None
...
@sensitive_variables()
def decrypt(token: str | None, keys: list[str] | None = None) -> str | None:
    if token is None:
        return None
    try:
...
@sensitive_variables()
def reencrypt_to_primary(
    token: str | None, keys: list[str] | None = None
) -> str | None:
