...
def notify_owner(user, event, **context):
    ...
    from onadata.apps.api.tasks import send_two_factor_changed_email_async
    ...
    try:
        send_two_factor_changed_email_async.apply_async(
            args=[
                user.email,
        ...
        # Logged so a degraded broker dropping security notifications is visible.
        logger.exception(
            "Could not enqueue the two-factor '%s' notification for user %s",
            event,
            user.pk,
        )
...
def _alert_on_repeated_failures(user):
    ...
    count = safe_cache_incr(key)
    if count is None:
        # An unreachable cache disables the alert; log it so a dead cache does
        # not silently turn off brute-force alerting.
        logger.warning(
            "Two-factor failure counter unavailable for user %s; "
            "repeated-failure alert may not fire.",
            user.pk,
        )
        return
    if count == threshold:
