...
from onadata.libs.utils.csv_import import get_submission_meta_dict, get_columns_by_type
...
class CSVImportTestCase(TestBase):
    ...
    def test_submit_csv_and_rollback(self):
        xls_file_path = os.path.join(
            settings.PROJECT_ROOT, "apps", "main", "tests", "fixtures", "tutorial.xls"
        )
    ...
    def test_csv_with_repeats_import(self):
        self.xls_file_path = os.path.join(
            self.this_directory, "fixtures", "csv_export", "tutorial_w_repeats.xls"
        )
        repeats_csv = open(
            os.path.join(
    ...
    def test_get_columns_by_type(self):
        ...
        self._publish_xls_file(self.xls_file_path)
        xform = XForm.objects.get()
        columns = get_columns_by_type(["date"], json.loads(xform.json))
        self.assertEqual(
