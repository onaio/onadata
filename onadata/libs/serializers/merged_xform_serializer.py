...
def minimum_two_xforms(value):
    ...
    if len(set(value)) != len(value):
        raise serializers.ValidationError(_("This field should have unique xforms"))
...
class XFormListField(serializers.ManyRelatedField):
    ...
    def to_representation(self, iterable):
        return [
            dict(i)
            for i in XFormSerializer(iterable, many=True, context=self.context).data
        ]
...
class MergedXFormSerializer(serializers.HyperlinkedModelSerializer):
    ...
    name = serializers.CharField(max_length=XFORM_TITLE_LENGTH, write_only=True)
