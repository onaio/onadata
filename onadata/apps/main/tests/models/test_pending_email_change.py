...
class PendingEmailChangeTests(TestCase):
    """Test the PendingEmailChange model and its OTP helpers."""

    def setUp(self):
        self.user = User.objects.create(username="bob", email="bob@x.com")

    def test_start_generates_6_digit_and_hashes(self):
        """start() issues a 6-digit code, normalizes the email and stores
        only a hash of the code."""
    ...
    def test_wrong_code_counts_attempt_and_fails(self):
        """A wrong code fails verification and increments the attempt count."""
    ...
    def test_expired_fails(self):
        """An expired code fails verification even when correct."""
    ...
    def test_over_max_attempts_fails(self):
        """Once MAX_ATTEMPTS wrong guesses are used up, even the correct
        code is rejected."""
        pec, code = PendingEmailChange.start(self.user, "n@x.com")
    ...
    def test_consume_returns_email_and_retires_row(self):
        """consume() with the right code returns the new email and deletes
        the pending row (single use)."""
        pec, code = PendingEmailChange.start(self.user, "New@X.com")
    ...
    def test_consume_wrong_code_returns_none_and_keeps_row(self):
        """consume() with a wrong code returns None and keeps the pending
