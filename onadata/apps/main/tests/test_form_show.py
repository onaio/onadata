...
from onadata.apps.viewer.views import data_export, export_list, map_view
from onadata.libs.utils.logger_tools import publish_xml_form
from onadata.libs.utils.user_auth import get_user_default_project, http_auth_string
...
class TestFormShow(TestBase):
    ...
    def test_dl_xlsx_xlsform(self):
        ...
        response = self.client.get(
            reverse(
                download_xlsform,
                kwargs={"username": self.user.username, "id_string": "exp_one"},
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
    ...
    def test_dl_xls_for_basic_auth(self):
        extra = {
            "HTTP_AUTHORIZATION": http_auth_string(
    ...
    def test_dl_xform_for_authenticated_non_owner(self):
        ...
        response = self.client.get(
            reverse(
                download_xform,
                kwargs={"username": "bob", "id_string": self.xform.id_string},
            )
        )
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(
            reverse(
                download_xform,
                kwargs={"username": "bob", "id_string": "random_id_string"},
            )
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_return_error_if_xform_not_found(self):
        map_url = reverse(
            map_view,
            kwargs={"username": self.user.username, "id_string": "random_string"},
        )
    ...
    def test_show_link_if_owner(self):
        ...
        self._publish_xls_file(
            os.path.join(os.path.dirname(__file__), "fixtures", "gps", "gps.xlsx")
        )
    ...
    def test_load_from_uuid(self):
        ...
        response = self.client.get(reverse(show, kwargs={"uuid": self.xform.uuid}))
    ...
    def test_xls_replace_markup(self):
        ...
        self.xform.shared = True
        self.xform.save()
        dashboard_url = reverse(profile, kwargs={"username": "bob"})
    ...
    def test_xform_delete(self):
        id_string = self.xform.id_string
        form_exists = (
            XForm.objects.filter(user=self.user, id_string=id_string).count() == 1
        )
        ...
        form_deleted = (
            XForm.objects.filter(user=self.user, id_string=id_string).count() == 0
        )
        ...
        xform_delete_url = reverse(
            delete_xform,
            kwargs={"username": self.user.username, "id_string": "random_id_string"},
        )
        response = self.client.post(xform_delete_url)
        self.assertEqual(response.status_code, 404)

    @patch(
        "onadata.apps.logger.views.get_object_or_404",
    ...
    def test_non_owner_cant_delete_xform(self):
        ...
        self._create_user_and_login("alice", "alice")
        self.client.post(xform_delete_url)
        form_deleted = XForm.objects.filter(user=bob, id_string=id_string).count() == 0
    ...
    def test_publish_xml_xlsform_download(self):
        ...
        response = self.client.get(
            reverse(
                download_xlsform,
                kwargs={"username": self.user.username, "id_string": "contributions"},
            ),
            follow=True,
        )
