...
class TestFormMetadata(TestBase):
    ...
    def _add_metadata(self, data_type="doc"):
        ...
        path = os.path.join(self.this_directory, "fixtures", "transportation", name)
    ...
    def test_adds_supporting_doc_on_submit(self):
        count = len(
            MetaData.objects.filter(object_id=self.xform.id, data_type="supporting_doc")
        )
        self._add_metadata()
        self.assertEqual(
            count + 1,
    ...
    def test_delete_cached_xform_metadata_object_on_save(self):
        ...
        self.assertIsNone(cache.get("{}{}".format(XFORM_METADATA_CACHE, self.xform.id)))
        self.assertEqual(count + 1, MetaData.objects.count())

    def test_adds_supporting_media_on_submit(self):
        count = len(MetaData.objects.filter(object_id=self.xform.id, data_type="media"))
        self._add_metadata(data_type="media")
        self.assertEqual(
            count + 1,
            len(MetaData.objects.filter(object_id=self.xform.id, data_type="media")),
        )

    def test_adds_mapbox_layer_on_submit(self):
        count = MetaData.objects.filter(
            object_id=self.xform.id, data_type="mapbox_layer"
        ).count()
        self.post_data = {}
    ...
    def test_delete_supporting_media(self):
        ...
        self._add_metadata(data_type="media")
        self.assertEqual(
            MetaData.objects.filter(object_id=self.xform.id, data_type="media").count(),
            count + 1,
        )
    ...
    def _add_mapbox_layer(self):
        ...
        post_data = {"map_name": "test_mapbox_layer", "link": "http://0.0.0.0:8080"}
    ...
    def test_user_source_edit_updates(self):
        ...
        response = self.client.post(
            self.edit_url, {"source": desc}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_media_url(self):
        ...
        self.assertEqual(count + 1, len(MetaData.objects.filter(data_type="media")))
