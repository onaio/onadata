...
class TestFormEnterData(TestBase):
    ...
    def _get_grcode_view_response(self):
        ...
        response = qrcode(request, self.user.username, self.xform.id_string)
    ...
    def test_public_with_link_to_share_toggle_on(self):
        ...
        response = self.client.post(
            self.perm_url, {"for_user": "all", "perm_type": "link"}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_enter_data_with_unavailable_id_string(self):
        url = reverse(
            enter_data,
            kwargs={"username": self.user, "id_string": "random_id_string"},
        )
