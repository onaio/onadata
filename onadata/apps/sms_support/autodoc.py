...
from onadata.apps.sms_support.tools import (
    DEFAULT_ALLOW_MEDIA, DEFAULT_DATE_FORMAT, DEFAULT_DATETIME_FORMAT,
    DEFAULT_SEPARATOR, MEDIA_TYPES)
...
def get_sample_data_for(question, json_survey, as_names=False):
    ...
    if xlsf_type == 'text':
        ...
        return safe_wrap(
            u' '.join([c.get('sms_option') for c in xlsf_choices][:1]))
    elif xlsf_type == 'select all that apply':
        return safe_wrap(
            u' '.join([c.get('sms_option') for c in xlsf_choices][:2]))
...
def get_helper_text(question, json_survey):
    ...
    def safe_wrap(value, xlsf_type=xlsf_type):
        value = (
            u'<span class="sms_autodoc_helper_type">%(type)s</span> '
...
def get_autodoc_for(xform):
    ...
    sms_allow_media = bool(
        json_survey.get('sms_allow_media', DEFAULT_ALLOW_MEDIA)
        or DEFAULT_ALLOW_MEDIA)
    ...
    for group in json_survey.get('children', {}):
        ...
        for question in group.get('children', {}):
            ...
            if sample is None:
                continue
            sample_name = get_sample_data_for(
                question, json_survey, as_names=True)
            ...
            helpers.append((sample_name, get_helper_text(
                question, json_survey)))
