...
"""Repair stored XForm JSON schemas after upgrading PyXForm to 4.x."""
...
def process_select_question(child, choices):
    ...
    if matching_key:
        ...
        logging.info(
            "Question '%s' (%s) mapped to existing choice key '%s'",
            child["name"],
            child["type"],
            matching_key,
        )
        ...
        logging.info(
            "Question '%s' (%s) added new choice key '%s'",
            child["name"],
            child["type"],
            child["name"],
        )
...
def update_xform_schema(apps, schema_editor):  # pylint: disable=unused-argument
    """Update the schema of all XForms after the PyXForm v4 upgrade.

    This migration is necessary because the PyXForm v4 upgrade introduced
    schema incompatibilities that cause the existing stored JSON schema of
    some XForms to be rejected by PyXForm. This migration fixes the schema
    of all XForms after the PyXForm v4 upgrade.
    """
...
class Migration(migrations.Migration):
    """Migration to repair XForm JSON schemas after the PyXForm v4 upgrade."""
