...
def import_instance(username, xform_path, photos, osm_files, status, raise_exception):
    """
    This callback is passed an instance of a XFormInstanceFS.
    See xform_fs.py for more info.
    """
    with django_file(
        xform_path, field_name="xml_file", content_type="text/xml"
    ) as xml_file:
        images = [
...
def import_instances_from_path(path, user, status="zip", is_async=False):
    def callback(xform_fs):
        ...
        """
        import_instance(
            user.username, xform_fs.path, xform_fs.photos, xform_fs.osm, status, True
        )
