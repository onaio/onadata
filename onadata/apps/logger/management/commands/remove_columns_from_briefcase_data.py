...
def _traverse_child_nodes_and_delete_column(
        xml_obj, column: str) -> None:
...
class Command(BaseCommand):
    help = _(
        'Delete specific columns from submission '
        'XMLs pulled by ODK Briefcase.')
    ...
    def handle(self, *args, **options):
        ...
        for count, submission_folder in enumerate(submission_folders, start=1):
            self.stdout.write(
                f'Modifying {submission_folder}. '
                f'Progress {count}/{total_files}')
            data = None
        ...
        self.stdout.write(
            f'Operation completed. Modified {modified_files} files.')
