...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        """Download forms and submissions from a server with Briefcase Aggregate API"""
        parser.add_argument(
            "--url", help=_("server url to pull forms and submissions"), required=True
        )
        parser.add_argument("-u", "--username", help=_("Username"), required=True)
        parser.add_argument("-p", "--password", help=_("Password"), required=True)
    ...
    def handle(self, *args, **kwargs):
        ...
        id_string = kwargs.get("formid")
        if username is None or password is None or to_username is None or url is None:
            self.stderr.write(
                "pull_from_aggregate -u uxsername -p password --to=username"
