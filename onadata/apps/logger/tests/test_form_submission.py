...
class TestFormSubmission(TestBase):
    ...
    def test_duplicate_form_id(self):
        ...
        project = Project.objects.create(
            name="another project", organization=self.user, created_by=self.user
        )
    ...
    def test_fail_with_ioerror_read(self, mock_pop):
        mock_pop.side_effect = IOError("request data read error")
    ...
    def test_fail_with_ioerror_wsgi(self, mock_pop):
        mock_pop.side_effect = IOError("error during read(65536) on wsgi.input")
    ...
    def test_edited_submission(self):
        ...
        handler.assert_called_once_with(
            instance=edited_instance, sender=Instance, signal=ANY
        )
        ...
        self.assertEqual(
            edited_instance.date_created, instance_history_1.submission_date
        )
        # check that '_last_edited' key is not in the json
    ...
    def test_form_submission_with_infinity_values(self):
        ...
        xls_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            "../tests/fixtures/infinity.xlsx",
        )
        self._publish_xls_file_and_set_xform(xls_file_path)
        xml_submission_file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../tests/fixtures/infinity.xml"
        )
