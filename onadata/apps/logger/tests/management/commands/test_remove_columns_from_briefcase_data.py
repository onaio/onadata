...
from onadata.apps.logger.management.commands.\
    remove_columns_from_briefcase_data import remove_columns_from_xml
...
class TestRemoveColumnsFromBriefcaseDataCommand(TestBase):
    def test_removes_correct_columns(self):
        ...
        instance = create_instance(
            self.user.username, BytesIO(xml_string.strip().encode('utf-8')),
            media_files=[]
        )
        ...
        expected_xml = (
            ...
            f"</{id_string}>")
        modified_xml = remove_columns_from_xml(
            instance.xml, ['first_name', 'photo'])
