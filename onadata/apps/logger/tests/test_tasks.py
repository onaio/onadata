...
from onadata.apps.logger.tasks import (
    ...
    register_instance_export_columns_async,
    register_xform_export_columns_async,
...
@patch("onadata.apps.logger.tasks.register_instance_export_columns")
class RegisterInstanceExportColumnsAsyncTestCase(TestBase):
    """Tests for register_instance_export_columns_async"""
    ...
    def test_register_columns(self, mock_register):
        ...
        register_instance_export_columns_async.delay(self.instance.pk)
        mock_register.assert_called_once_with(self.instance)

    @patch("onadata.apps.logger.tasks.register_instance_export_columns_async.retry")
    ...
    def test_invalid_pk(self, mock_logger, mock_register):
        ...
        register_instance_export_columns_async.delay(sys.maxsize)
        mock_register.assert_not_called()
        mock_logger.assert_called_once()


@patch("onadata.apps.logger.tasks.register_xform_export_columns")
class RegisterXFormExportColumnsAsyncTestCase(TestBase):
    """Tests for register_xform_export_columns_async"""
    ...
    @patch("onadata.apps.logger.tasks.register_xform_export_columns_async.retry")
    def test_retry_connection_error(self, mock_retry, mock_register):
        """ConnectionError exception is retried"""
        mock_retry.side_effect = Retry
        mock_register.side_effect = ConnectionError
    ...
    def test_invalid_pk(self, mock_logger, mock_register):
        ...
        register_xform_export_columns_async.delay(sys.maxsize)
        mock_register.assert_not_called()
        mock_logger.assert_called_once()
