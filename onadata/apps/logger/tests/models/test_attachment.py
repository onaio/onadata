...
from onadata.apps.logger.models.attachment import get_original_filename, upload_to
...
class TestAttachment(TestBase):
    ...
    def test_create_attachment_with_mimetype_more_than_50(self):
        ...
        with self.assertRaises(DataError):
            Attachment.objects.create(
                instance=self.instance, mimetype="a" * 120, media_file=media_file
            )

        pre_count = Attachment.objects.count()
        Attachment.objects.create(
            instance=self.instance, mimetype="a" * 100, media_file=media_file
        )
    ...
    def test_create_attachment_with_media_file_length_more_the_100(self):
        with self.assertRaises(ValueError):
            Attachment.objects.create(instance=self.instance, media_file="a" * 300)

        pre_count = Attachment.objects.count()
        Attachment.objects.create(instance=self.instance, media_file="a" * 150)
        self.assertEqual(pre_count + 1, Attachment.objects.count())
    ...
    def test_create_thumbnails_command(self):
        ...
        for attachment in Attachment.objects.filter(instance=self.instance):
            ...
            for size in ["small", "medium", "large"]:
                ...
                self.assertTrue(
                    default_storage.get_modified_time(thumbnail) > check_datetime
                )
                default_storage.delete(thumbnail)

    def test_get_original_filename(self):
        self.assertEqual(
            get_original_filename("submission.xml_K337n8u.enc"), "submission.xml.enc"
        )
        self.assertEqual(
            get_original_filename("submission.xml.enc"), "submission.xml.enc"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            get_original_filename("submission_random.enc"), "submission_random.enc"
        )
