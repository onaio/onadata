...
class TestKMLExport(TestBase):
    ...
    def test_kml_export(self):
        ...
        url = reverse(
            kml_export, kwargs={"username": self.user.username, "id_string": id_string}
        )
        response = self.client.get(url)
        instances = Instance.objects.filter(
            xform__user=self.user, xform__id_string=id_string, geom__isnull=False
        ).order_by("id")
        ...
        replacement_data = [
            ["{:,}".format(x) for x in [i.pk, i.point.x, i.point.y]] for i in instances
        ]
        ...
        with open(os.path.join(self.fixtures, "export.kml")) as f:
            ...
            self.assertMultiLineEqual(
                expected_content.strip(), response.content.decode("utf-8").strip()
            )
