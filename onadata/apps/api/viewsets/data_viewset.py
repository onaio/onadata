...
from onadata.libs.pagination import InstancePageNumberPagination
...
class DataViewSet(
    ...
    pagination_class = InstancePageNumberPagination
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if isinstance(self.object, XForm):
            if not instance_ids:
                ...
                return Response(
                    data={
                        "message": "%d records were deleted"
            ...
            if request.user.has_perm(CAN_DELETE_SUBMISSION, self.object.xform):
                ...
                raise PermissionDenied(
                    _(u"You do not have delete " u"permissions.")
                )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if _format == "json" or _format is None or _format == "debug":
            ...
            raise ParseError(
                _(
                    u"'%(_format)s' format unknown or not implemented!"
                    % {"_format": _format}
                )
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        if is_public_request:
            ...
            self.object_list = (
                Instance.objects.filter(xform_id__in=pks, deleted_at=None)
                .only("json")
                .order_by("id")
            )
            ...
            if not_tagged and isinstance(not_tagged, six.string_types):
                ...
                self.object_list = self.object_list.exclude(
                    tags__name__in=not_tagged
                )

        if (
            export_type is None or export_type in ["json", "jsonp", "debug"]
