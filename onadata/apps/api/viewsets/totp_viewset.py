...
#: The step-up operations this viewset owns. The single source of truth for
#: the audience strings the endpoints pass, and always recognised by
#: ``_known_audiences`` so a deployment's ``TWO_FACTOR_STEP_UP_AUDIENCES`` can
#: only ADD account-action gates, never drop a lifecycle one and silently break
#: its grants.
AUDIENCE_ENROLL_START = "enroll-start"
AUDIENCE_DISABLE = "disable"
AUDIENCE_RECOVERY_GENERATE = "recovery-generate"
AUDIENCE_RECOVERY_VIEW = "recovery-view"
LIFECYCLE_AUDIENCES = frozenset(
    {
        AUDIENCE_ENROLL_START,
        AUDIENCE_DISABLE,
        AUDIENCE_RECOVERY_GENERATE,
        AUDIENCE_RECOVERY_VIEW,
    }
)
...
def _issue_grant(user, audience: str) -> str:
    ...
    Grants live in the cache, as do the lockout and repeated-failure counters,
    so the deployment must run a cache shared across workers (Redis/memcached);
    a per-process cache would fragment all three and weaken them.
    """
...
def _known_audiences() -> frozenset[str]:
    ...
    test's -- takes effect.
    """
    return LIFECYCLE_AUDIENCES | frozenset(
        getattr(settings, "TWO_FACTOR_STEP_UP_AUDIENCES", ())
    )
...
def _missing_code_response() -> Response:
    """The refusal for a request that carries no code.

    An absent code is an incomplete request, not a wrong guess -- as
    ``_require_password`` treats an absent password -- so callers return this
    before recording a failure or spending the allowance.
    """
    return Response(
        {"error": "A current code is required.", "reason": "code_required"},
...
def _totp_device(
    user, confirmed: bool = True, lock: bool = False
) -> EncryptedTOTPDevice | None:
...
def _recovery_device(user, lock: bool = False) -> EncryptedRecoveryDevice | None:
...
def _method_for_code(code: str) -> str:
    ...
    return (
        "totp"
        if code.isdigit() and len(code) == EncryptedTOTPDevice.digits
        else "recovery"
    )
...
class TOTPViewSet(ViewSet):
    ...
    # Three credentials: an API token and two cookie-borne ones (the Django
    # session and the ambient SSO cookie/header). SSOHeaderAuthentication runs
    # first, so an ambient SSO cookie is authenticated here rather than by
    # SessionAuthentication; initial() -> _enforce_cookie_sso_csrf applies the
    # CSRF check to it, as SessionAuthentication does for the session cookie.
    ...
    def _require_code(self, request, audience: str):
        ...
        if _verify_code(request.user, code):
            clear_verification_failures(request.user)
            return None
    ...
    def enroll_start(self, request):
        ...
        if _has_second_factor(request.user):
            ...
            refusal = self._require_code(request, AUDIENCE_ENROLL_START)
        else:
    ...
    def disable(self, request):
        ...
        refusal = self._require_code(request, AUDIENCE_DISABLE)
        if refusal is not None:
            return refusal
        with transaction.atomic():
    ...
    def recovery_generate(self, request):
        ...
        refusal = self._require_code(request, AUDIENCE_RECOVERY_GENERATE)
        if refusal is not None:
            return refusal
        codes = _regenerate_recovery_codes(request.user)
        audit_log(
    ...
    def recovery_view(self, request):
        ...
        refusal = self._require_code(request, AUDIENCE_RECOVERY_VIEW)
        if refusal is not None:
            return refusal
        audit_log(
    ...
    def verify(self, request):
        ...
        # An empty method means any-factor.
