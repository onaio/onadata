...
def _incoming_eu_consent(data):
    ...
    if isinstance(raw, str):
        try:
            ...
            return None
    return eu_consent_record(raw)


def _stored_eu_consent(profile):
    return eu_consent_record(profile.metadata or {})
...
class UserProfileViewSet(
    ...
    def _step_up_refusal(self, request):
        ...
        if "require_auth" in request.data:
            # Parsed as the serializer will parse it, so a value the two read
            # differently cannot change the setting without being seen here.
            incoming_require_auth = serializers.BooleanField().to_internal_value(
                request.data.get("require_auth")
            )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if request.data.get("overwrite") == "false":
            ...
            consent_before = deepcopy(eu_consent_record(metadata))
            ...
            if eu_consent_record(metadata) != consent_before:
