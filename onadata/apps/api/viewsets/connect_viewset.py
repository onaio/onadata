...
from onadata.libs.stepup.policy import (
    GATE_REGENERATE_API_KEY,
    GATE_REGENERATE_ODK_TOKEN,
)
...
class ConnectViewSet(
    ...
    def odk_token(self, request, *args, **kwargs):
        ...
        the old one carried, so it is gated like ``regenerate_auth_token`` --
        under its own audience, so a grant for one rotation cannot perform the
        other. The GET is the ordinary fetch-or-provision flow and stays open.
        """
        user = request.user
        ...
        if request.method == "POST":
            refusal = self.check_step_up(request, GATE_REGENERATE_ODK_TOKEN)
            if refusal is not None:
                return refusal
