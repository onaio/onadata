...
class StepUpViewSet(StepUpCallbackMixin, ViewSet):
    ...
    def callback(self, request):
        ...
        if grant is None:
            # The same reason code the popup receives. It names the check that
            # failed, not what the user did -- a cancelled prompt and a provider
            # with no MFA configured can look alike here -- so show one generic
            # failure rather than branching on it.
            return Response(
