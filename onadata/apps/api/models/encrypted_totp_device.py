...
class EncryptedTOTPDevice(TimestampMixin, ThrottlingMixin, Device):
    ...
    def config_url(self):
        ...
        Decorated so the plaintext seed it materialises is scrubbed from a
        traceback frame if any statement here raises.
        """
        ...
        with transaction.atomic():
            ...
            try:
                ...
                try:
                    ...
                    return False
                totp = TOTP(key, locked.step, locked.t0, locked.digits, locked.drift)
                totp.time = time.time()
                verified = totp.verify(token, locked.tolerance, locked.last_t + 1)
            if verified:
