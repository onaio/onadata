...
class EncryptedRecoveryDevice(TimestampMixin, ThrottlingMixin, Device):
    ...
    def unspent_codes(self) -> list[str]:
        ...
        Skips a code the current keys cannot read, as verify_token does.
        """
        codes = []
        for code in self.codes.filter(used=False):
            try:
                codes.append(decrypt(code.encrypted_code))
    ...
    def verify_token(self, token: object) -> bool:
        # A non-string token cannot be a recovery code; treat it as a wrong
        # guess (as the authenticator device does) rather than raising.
        target = _normalize(token) if isinstance(token, str) else ""
        ...
        with transaction.atomic():
            ...
            if not verify_allowed:
                return False
            for code in (
                locked.codes.select_for_update().filter(used=False).order_by("id")
            ):
                try:
                    stored = _normalize(decrypt(code.encrypted_code)).encode()
                except FieldEncryptionError:
                    # Skip an undecryptable code (e.g. a key dropped mid
                    # rotation) rather than failing the whole set: it can never
                    # match, so the still-valid codes remain usable.
                    logger.error(
                        "Cannot decrypt a recovery code; check "
                        "TWO_FACTOR_FIELD_ENCRYPTION_KEYS."
                    )
