...
class TestRetrieveOrgOrProjectListCommand(TestAbstractViewSet):
    def test_retrieve_org_or_project_list(self):
        ...
        user = self._create_user_profile({"username": "alice"}).user
        share_data = {
        ...
        out = StringIO()
        call_command("retrieve_org_or_project_list", stdout=out)
        ...
        self.assertEqual(expected_data, json.loads(out.getvalue()))

        out = StringIO()

        call_command(
            "retrieve_org_or_project_list", project_ids=f"{self.project.id}", stdout=out
        )
        self.assertEqual(expected_project_data, json.loads(out.getvalue()))
        ...
        self.assertEqual(expected_org_data, json.loads(out.getvalue()))
