...
class TestCaseInsensitiveUserQuery(TestCase):
    ...
    def setUpClass(cls):
        ...
        # Re-run the specific migration tailored to this test. This is because
        # although the migration is applied, its changes do not show in the test database
        with connection.cursor() as cursor:
            # Re-create the collations
            cursor.execute("""
            ...
            # Reapply the collation to username
            cursor.execute("""
                ALTER TABLE auth_user 
                ALTER COLUMN username TYPE varchar(150) COLLATE "case_insensitive";
            """)
