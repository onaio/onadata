...
class TestTOTPViewSet(TestAbstractViewSet):
    ...
    def _sso_cookie(self, user=None):
        """The SSO token as the ambient cookie a browser sends, not the header."""
        config = settings.OPENID_CONNECT_VIEWSET_CONFIG
        return jwt.encode(
            {"email": (user or self.user).email},
            config["JWT_SECRET_KEY"],
    ...
    #: CSRF is skipped under the default test factory, which flags every
    #: request exempt; these cases need the real check, so they build requests
    #: through a factory that enforces it, as a live request would.
    csrf_factory = APIRequestFactory(enforce_csrf_checks=True)

    def test_cookie_borne_sso_post_is_refused_without_a_csrf_token(self):
        """A cross-site page can ride the ambient SSO cookie but cannot set a
        CSRF token, so the cookie path must demand one."""
        view = TOTPViewSet.as_view({"post": "enroll_start"})
        request = self.csrf_factory.post("/", data={})
        request.COOKIES["SSO"] = self._sso_cookie()
        response = view(request)
        self.assertEqual(response.status_code, 403)
        self.assertIn("CSRF", str(response.data.get("detail", "")))

    def test_header_borne_sso_post_needs_no_csrf_token(self):
    ...
    def test_cookie_borne_sso_post_passes_with_a_valid_csrf_token(self):
    ...
    @override_settings(DEBUG=False)
    def test_decrypted_secrets_are_scrubbed_from_traceback_locals(self):
        """A 500 inside the encryption helpers must not carry the plaintext.

        Seeds and codes are minted server-side, so they never pass through the
        POST body the previous test covers -- they live only as frame locals.
        ``sensitive_variables`` on those functions cleanses the whole frame; a
        500 in ``encrypt`` (no key configured) must not surface its ``value``.
        Without the decorator this fails, since the local is reported verbatim.
        """
        import sys

        from onadata.libs.utils import field_encryption

        secret = "s3cret-seed-value"  # nosec B105 - test fixture, not a credential
        with override_settings(TWO_FACTOR_FIELD_ENCRYPTION_KEYS=[]):
            try:
                field_encryption.encrypt(secret)
                self.fail("encrypt should raise without a configured key")
            except field_encryption.FieldEncryptionError:
        ...
        encrypt_frame = next(f for f in frames if f.f_code.co_name == "encrypt")
        reported = dict(
            SafeExceptionReporterFilter().get_traceback_frame_variables(
                None, encrypt_frame
            )
        )
        self.assertIn("value", reported, "the local should still be listed")
        self.assertNotIn(secret, str(reported["value"]))
    ...
    def test_viewing_recovery_codes_notifies_the_account_owner(self):
        """Disclosing the durable set emails the owner, as regenerating it does,
        so one intercepted code cannot buy the recovery codes without a signal."""
        device = self._enroll()
        with next_totp_window():
            self._post_with_api_key(
                "recovery_generate", {"code": current_code(device.key)}
        ...
        self.assertTrue(
            ...
            "viewing the recovery set did not notify the account owner",
        )
    ...
    def test_verify_treats_an_absent_code_as_incomplete_not_a_failure(self):
        """An empty code is an incomplete request, not a wrong guess: it must
        neither alert the owner nor spend the shared lockout allowance."""
        cache.clear()
        self.addCleanup(cache.clear)
        self._enroll()
        mail.outbox = []
        ...
        for _ in range(settings.MAX_LOGIN_ATTEMPTS + 2):
            ...
            self.assertEqual(response.status_code, 403)
            self.assertEqual(response.data["reason"], "code_required")

        self.assertEqual(
            [m for m in mail.outbox if self.user.email in m.to],
            [],
            "an absent code triggered the repeated-failure alert",
        )

    @override_settings(MAX_LOGIN_ATTEMPTS=3)
    def test_a_code_gated_action_treats_an_absent_code_as_incomplete(self):
        """The same for the step-up path (disable): no code and no grant is an
        incomplete request, so it leaves the factor and the allowance alone."""
        cache.clear()
        self.addCleanup(cache.clear)
        self._enroll()
        mail.outbox = []
...
class SecureEnrolmentDefaultTestCase(TestAbstractViewSet):
    """First enrolment demands the account password, so an SSO credential
    alone cannot enrol a factor and lock the owner out of their own second
    factor."""
