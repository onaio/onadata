...
class TestOSMViewSet(TestAbstractViewSet):
    ...
    def _publish_osm_with_submission(self):
        ...
        self.fixtures_dir = osm_fixtures_dir = os.path.realpath(
        ...
        request = self.factory.get("/", {"instances_with_osm": "True"}, **self.extra)
    ...
    def test_data_retrieve_instance_osm_format(self):
        ...
        self.assertEqual(response["Location"], "http://testserver/api/v1/osm.json")
    ...
    def test_osm_csv_export(self):
        ...
        request = self.factory.get("/", data={"include_images": False}, **self.extra)
    ...
    def test_process_error_osm_format(self):
        ...
        self.assertContains(
            response,
            "<error>No data matches with given query.</error>",
            status_code=404,
        )
    ...
    def test_save_osm_data_with_non_existing_media_file(self):
        ...
        submission_path = os.path.join(osm_fixtures_dir, "instance_a.xml")
        media_file = open(os.path.join(osm_fixtures_dir, "OSMWay234134797.osm"))
