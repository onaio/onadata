...
class TestStatsViewSet(TestBase):
    ...
    def test_submissions_stats(self):
        ...
        data = {"detail": "Expecting `group` and `name` query parameters."}
        self.assertEqual(response.data, data)
    ...
    def test_form_list_select_one_choices_multi_language(self):
        paths = [
            os.path.join(self.this_directory, "fixtures", "good_eats_multilang", x)
        ...
        view = SubmissionStatsViewSet.as_view({"get": "retrieve"})
        formid = self.xform.pk
        request = self.factory.get("/?group=rating", **self.extra)
        response = view(request, pk=formid)
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.data, list)
        data = [{"count": 1, "rating": "Nothing Special"}]
    ...
    def test_form_list_select_one_choices(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.data, list)
        data = [{"count": 2, "gender": "Female"}, {"count": 1, "gender": "Male"}]
        self.assertEqual(
            sorted(data, key=lambda k: k["gender"]),
    ...
    def _tutorial_form_submission(self):
        ...
        instance_paths = [
            os.path.join(tutorial_folder, "instances", i)
    ...
    def test_bad_field(self):
        self._contributions_form_submissions()
        view = StatsViewSet.as_view({"get": "retrieve"})
        request = self.factory.get("/?method=median&field=INVALID", **self.extra)
        formid = self.xform.pk
        response = view(request, pk=formid)
        self.assertEqual(response.status_code, 400)
        ...
        data = [
            {
                ...
                "url": "http://testserver/api/v1/stats/%s" % formid,
            }
        ]
        self.assertEqual(data, response.data)
