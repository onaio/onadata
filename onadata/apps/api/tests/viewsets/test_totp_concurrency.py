...
class WizardVerifyConcurrencyTestCase(TransactionTestCase):
    """The login wizard verifies through the device's own ``verify_token``,
    without the row lock the viewset helpers take -- so ``verify_token`` must
    lock internally, or a single-use code is spent twice by racing submissions.
    """

    def setUp(self):
        super().setUp()
        self.user = User.objects.create_user(
            "wizardrace", "wizardrace@example.com", "pw-9F3kz"
        )

    def _spend_concurrently(self, verify):
        barrier = threading.Barrier(CONCURRENT)
        results = []
        guard = threading.Lock()
    ...
    def test_a_recovery_code_verify_token_spends_once(self):
        device = EncryptedRecoveryDevice.objects.create(
            user=self.user, name=RECOVERY_DEVICE_NAME, confirmed=True
        )
        EncryptedRecoveryCode.objects.create(
        ...
        # Each worker loads its own unlocked instance, as the wizard does.
        results = self._spend_concurrently(
            lambda: EncryptedRecoveryDevice.objects.get(pk=device.pk).verify_token(
                RECOVERY_CODE
            )
        )
        ...
        self.assertEqual(len(results), CONCURRENT, "a worker did not finish")
        self.assertEqual(
            sum(results), 1, f"the code was accepted {sum(results)} times"
        )

    def test_a_totp_code_verify_token_spends_once(self):
        device = EncryptedTOTPDevice.objects.create(
            user=self.user, name=TOTP_DEVICE_NAME, confirmed=True
        )
        code = f"{totp(device.bin_key, device.step, device.t0):06d}"

        results = self._spend_concurrently(
            lambda: EncryptedTOTPDevice.objects.get(pk=device.pk).verify_token(code)
        )

        self.assertEqual(len(results), CONCURRENT, "a worker did not finish")
        self.assertEqual(sum(results), 1, f"the code verified {sum(results)} times")
