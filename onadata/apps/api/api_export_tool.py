...
def create_export_async(xform, export_type, query=None, force_xlsx=False,
                         options=None):
        """
...
def export_async_export_response(request, xform, export):
    """
    ...
    if export.status == Export.SUCCESSFUL:
        if export.export_type != Export.EXTERNAL_EXPORT:
            ...
            if str2bool(request.QUERY_PARAMS.get(remove_group_key)):
    ...
    try:
        if export_type == Export.EXTERNAL_EXPORT:
            ...
            if "remove_group_name" in request.QUERY_PARAMS:
                remove_group_name = \
                    str2bool(request.QUERY_PARAMS["remove_group_name"])
...
def custom_response_handler(request, xform, query, export_type,
    ...
    remove_group_name = str2bool(request.GET.get('remove_group_name'))
