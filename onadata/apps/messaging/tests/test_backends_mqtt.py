...
from onadata.apps.messaging.backends.mqtt import (
    MQTTBackend,
    get_payload,
    get_target_metadata,
)
from onadata.apps.messaging.constants import PROJECT, XFORM
from onadata.apps.messaging.tests.test_base import _create_message, _create_user
...
class TestMQTTBackend(TestCase):
    ...
    def test_mqtt_get_topic(self):
        ...
        expected = "/{topic_root}/{target_name}/{target_id}/messages/publish".format(
            topic_root="onadata", target_name="user", target_id=to_user.id
        )
    ...
    def test_get_target_metadata(self):
        ...
        xform_metadata = {"id": 1337, "name": "Test Form", "form_id": "Test_Form_ID"}
        self.assertEqual(
        ...
        self.assertEqual(
            json.dumps(project_metadata), json.dumps(get_target_metadata(project))
        )
    ...
    def test_mqtt_get_payload(self):
        ...
        payload = {
            ...
            "payload": {
                ...
                "context": {
                    ...
                    "metadata": {"id": to_user.pk, "name": to_user.get_full_name()},
                    "verb": "message",
                },
                "message": "I love oov",
            },
        ...
        self.assertEqual(
            json.dumps(expected_payload), get_payload(instance, verbose_payload=False)
        )
